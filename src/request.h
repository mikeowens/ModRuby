...
class Request
{
  ...
  public:
    ...
    /// @return Sets up CGI environmental variables in the subprocess_env table
    ...
    /// of handling. Generally modules should DECLINE any request methods they
    /// do not handle. Prior to aborting the handler like this the handler
    /// should set r->allowed to the list of methods that it is willing to
    /// handle. This bitvector is used to construct the "Allow:" header required
    /// for OPTIONS requests, and HTTP_METHOD_NOT_ALLOWED and
    ...
    /// *not* implement GET should probably return
    ...
    /// @param sent_pw a reference to a string pointer into the request where
    ///   the password is
    /// @return 0 (OK) if it set the 'pw' argument (and assured a correct value
    ///   in r->user); otherwise it returns an error code, either
    ///   HTTP_INTERNAL_SERVER_ERROR if things are really confused,
    ///   HTTP_UNAUTHORIZED if no authentication at all seemed to be in use, or
    ///   DECLINED if there was authentication but it wasn't Basic (in which
    ///   case, the caller should presumably decline as well).
    ...
    /// @return the document root from the configuration (not necessarily the
    /// active one for this request)
    inline const char* document_root() const { return ap_document_root(_req); }

    /// @return Lookup the login name of the remote user.  Undefined if it
    ...
    /// @param str the custom response - it can be a static string, the full
    ...
    /// @brief This function is designed for things like actions or CGI scripts,
    ///   when using AddHandler, and you want to preserve the content type
    ...
    /// Parses a query argument string, url-decodes key/values, and merges into
    /// table.
