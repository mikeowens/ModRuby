...
do
  ...
  # XML file for dblatex documents

  if [ -f $out_file_latex ]; then
    # Then stat it
    out_file_latex_mtime=$(stat -c "%Y" $out_file_latex)
  else
      # Otherwise it doesn't exist and we will just use zero, which will always
      # be less than the mtime of the input file, therefore forcing it to be
      # processed.
      out_file_latex_mtime=0
  fi

  # If source file is newer than html-sized file
  if [ $in_file_mtime -gt $out_file_latex_mtime ]; then

      # Regenerate the html-ized file
      echo "$doc -> $out_file_latex"
      ret=$(./scripts/code2xml.sh $doc 2> /dev/null > $out_file_latex )
